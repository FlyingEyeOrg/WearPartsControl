...
- 模切分条工序当前通过 `ToolChange` 主数据下拉选择换刀类型；易损件新增/编辑窗口不再绑定换刀类型，更换页按当前工序选择并回退到该易损件最近一次选择的换刀类型。更换条码仍必须包含所选工具编码。
...
- 登录窗口支持两种模式：默认刷卡登录；导入旧配置后如果 `UseUserNumber=true`，则切换为工号登录。
...
- 客户端信息页中的“测试 PLC 连接”按钮会按当前表单值直接发起连接测试；即使当前已经连通也允许重复测试，连接成功后会同步刷新全局 PLC 连接状态。
- 客户端信息页会显式展示“易损件监控状态”；“开启/关闭易损件监控”按钮会写入 `IsWearPartMonitoringEnabled`，关闭后后台监控周期任务会跳过执行，重新开启后会立即触发一次监控。
