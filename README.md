...
- 如果已经配置 `ClientApp` 且上次关闭软件时仍开启了易损件监控，应用启动时会按当前资源号自动建立 PLC 连接；如果监控原本处于关闭状态，则启动和后续切换到设备基础信息页都不会隐式触发 PLC 自动连接。
...
- 更换易损件时，程序当前只向 PLC 写入“当前寿命”；预警寿命和停机寿命改由设备侧自行维护，不再由客户端回写。
